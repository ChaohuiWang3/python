...
#bug: the building_num is in range 0,1200//spacing, so there are 12 buildings, but the screen could only show 10 buildings
#should happen: all buildings show on the screen
CANVAS_WIDTH = 1200
CANVAS_HEIGHT = 700
SPACING = 100
speed = 0.0
...
def is_inside_building(building_num, pos, building_width, building_heights, building_xpos):
    ...
    #bug: while the building height is 0 the plane still collides the building
    #should happen: as the building height is 0 the function does not check whether the plane is inside building
...
def move_plane(pos):
    ...
    if pos[0] < -plane_width:
        ...
        #bug: the plane does not ouccr at the rightmost of the screen
        #should happen: the plane should ouccr at the rightmost of the screen
        pos[0] += CANVAS_WIDTH + plane_width
        pos[1] = pos[1] + 40
        #check we don't go off the bottom of the screen

        # bug: the plane continues to move below the bottom of the screen
        #should happen: the game finishes as the plane touches the bottom of the screen
        if pos[1] > CANVAS_HEIGHT - 32:
            pos[1] = CANVAS_HEIGHT - 32
...
def check_bomb(canvas, bomb_pos, building_width, building_heights, building_xpos, building_rects):
    ...
    for building_num in range(0, 1200//SPACING):
        ...
        #bug: if the bomb does not touch the building the plane could not drop bomb anymore
        #should happen: if the bomb does not touch the building the plane could continue to drop bomb
        if bomb_pos[1] > CANVAS_HEIGHT:
            explode()
